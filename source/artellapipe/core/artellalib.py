...
def update_artella_paths():
    ...
    artellapipe.logger.debug('Updating Artella paths from: {0}'.format(artella_folder))
    if artella_folder is not None and os.path.exists(artella_folder):
        for subdir, dirs, files in os.walk(artella_folder):
            if subdir not in sys.path:
                artellapipe.logger.debug('Adding Artella path: {0}'.format(subdir))
...
def get_artella_data_folder():
    ...
    if artella_app_version is not None:
        ...
        if len(artella_folder) == 1:
            ...
            artellapipe.logger.info('Artella folder not found!')

    artellapipe.logger.debug('ARTELLA FOLDER: {}'.format(artella_folder))
...
def launch_artella_app():
    ...
    if os.name == 'mac':
        artellapipe.logger.info('Launch Artella App: does not supports MAC yet')
    ...
    artellapipe.logger.info('Artella App File: {0}'.format(artella_app_file))

    if os.path.isfile(artella_app_file):
        artellapipe.logger.info('Launching Artella App ...')
        artellapipe.logger.debug('Artella App File: {0}'.format(artella_app_file))
    ...
    try:
        for proc in psutil.process_iter():
            if proc.name() == defines.ARTELLA_APP_NAME + '.exe':
                artellapipe.logger.debug('Killing Artella App process: {}'.format(proc.name()))
                proc.kill()
        return True
    except:
    ...
    def get_handle_msg(json_msg):
        if tp.is_houdini():
            try:
                ...
                artellapipe.logger.warning('Unknown command!')
        else:
...
def load_artella_maya_plugin():
    ...
    if tp.is_maya():
        artellapipe.logger.debug('Loading Artella Maya Plugin ...')
...
def get_cms_uri_current_file():
    ...
    artellapipe.logger.debug('Getting CMS Uri of file {0}'.format(current_file))

    cms_uri = artella.getCmsUri(current_file)
    if not cms_uri:
        artellapipe.logger.error('Unable to get CMS uri from path: {0}'.format(current_file))
        return False

    artellapipe.logger.debug('Retrieved CMS uri: {0}'.format(cms_uri))
...
def get_status_current_file():
    ...
    current_file = tp.Dcc.scene_name()
    artellapipe.logger.debug('Getting Artella Status of file {0}'.format(current_file))

    status = get_status(current_file)
    artellapipe.logger.debug('{0} STATUS -> {1}'.format(current_file, status))
...
def is_published(file_path):
    ...
    if meta.get('status') != 'OK':
        artellapipe.logger.info('Status is not OK: {}'.format(meta))
        return False
    ...
    if rsp:
        if isinstance(rsp, artellaclasses.ArtellaDirectoryMetaData):
            if rsp.header.status != 'OK':
                artellapipe.logger.info('Status is not OK: {}'.format(rsp))
                return False, False

            file_path = rsp.header.file_path or ''
            if not file_path or file_path == '':
                artellapipe.logger.info('File path not found in response: {}'.format(rsp))
                return False, False

            for ref, ref_data in rsp.references.items():
                file_data = ref_data.path
                if not file_data:
                    artellapipe.logger.info('File data not found in response: {}'.format(rsp.get('data', {}), ))
                    return
                if ref_data.locked:
...
def lock_file(file_path=None, force=False):
    ...
    if not file_path:
        artellapipe.logger.error('File {} cannot be locked because it does not exists!'.format(file_path))
        return False
    ...
    if in_edit_mode and not is_locked_by_me:
        ...
        if not force and not in_edit_mode:
            ...
            if artellapipe.project:
                artellapipe.project.message(msg)
            result = tp.Dcc.confirm_dialog(title='Artella Pipeline - Lock File', message=msg, button=['Yes', 'No'], cancel_button='No', dismiss_string='No')
        if result != 'Yes':
            return False
...
def upload_file(file_path, comment):
    ...
    if rsp.get('status', {}).get('meta', {}).get('status') != 'OK':
        ...
        artellapipe.logger.info(msg)
        tp.Dcc.warning(msg)
        tp.Dcc.confirm_dialog(title='Artella Pipeline -  Failed to Upload Bug. Restart DCC please!', message=msg, button=['OK'])
        return False
...
def can_unlock(file_path):
    ...
    if type(asset_status) == artellaclasses.ArtellaHeaderMetaData:
        artellapipe.logger.debug('File {} is not uploaded on Artella yet, so you cannot unlock it!')
        return False
...
def unlock_file(file_path):
    ...
    if not can_unlock(file_path=file_path):
        artellapipe.logger.debug('Impossible to unlock file. File is locked by other user!')
        return
...
def upload_new_asset_version(file_path=None, comment='Published new version with Artella Pipeline', skip_saving=False):
    ...
    if file_path is not None and file_path != '':
        ...
        if not skip_saving:
            ...
            if tp.is_maya():
                ...
                if helpers.file_has_student_line(filename=file_path):
                    ...
                    if helpers.file_has_student_line(filename=file_path):
                        artellapipe.logger.error('After updating model path the Student License could not be fixed again!')
                        return False
        ...
        if artellapipe.project:
            artellapipe.project.message(msg)
        if comment is None:
            result = tp.Dcc.confirm_dialog(title='Artella Pipeline - Save New Version on Artella Server', message=msg, button=['Save', 'Cancel'],
        ...
        if 'status' in rsp and 'meta' in rsp['status'] and rsp['status']['meta']['status'] != 'OK':
            artellapipe.logger.info('Make new version response: {}'.format(rsp))
...
def within_artella_scene():
    ...
    artellapipe.logger.debug('Current scene name: {}'.format(current_scene))
