...
class ArtellaSequence(object):
    ...
    def get_info(self, force_update=False):
        ...
        if not self._info or force_update:
            ...
            if not isinstance(shot_info, artellaclasses.ArtellaDirectoryMetaData):
                artellapipe.logger.warning('Shot {} has has no info inside!'.format(self.get_complete_name()))
                return
    ...
    def update_files(self, status=defines.ARTELLA_SYNC_WORKING_ASSET_STATUS, force_update=False):
        ...
        for file_name in self.DEFAULT_FILES:
            file_folder = self._project.format_template('{}_folder'.format(file_name), {'sequence_name': self._name, 'sequence_index': self._id})
            self._files[file_name] = dict()
            self._files[file_name]['path'] = file_folder

            if status == defines.ARTELLA_SYNC_WORKING_ASSET_STATUS:
                file_folder_working = self._project.format_template('{}_working_folder'.format(file_name), {'sequence_name': self._name, 'sequence_index': self._id})
                if not file_folder_working:
                    artellapipe.logger.warning('Impossible to retrieve {} folder for Sequence {} File: "{}"'.format(defines.ARTELLA_SYNC_WORKING_ASSET_STATUS, self._name, file_name))
                    return
                ...
                if not file_folder_working_info:
                    artellapipe.logger.warning('Working file for: {} | {} does not exists in Artella server!'.format(file_name, file_folder_working))
                    return
                if not isinstance(file_folder_working_info, artellaclasses.ArtellaDirectoryMetaData):
                    artellapipe.logger.warning('Artella information retrieve from Artella path: "{}" is not valid. Contact TD!'.format(file_folder_working))
                    continue

                layout_files = list()
                if file_folder_working_info.references:
                    for ref_name, ref_data in file_folder_working_info.references.items():
                        layout_files.append(path_utils.clean_path(os.path.join(file_folder_working, ref_name.split('/')[-1])))
                ...
                if len(layout_files) > 0:
                    ...
                    self._files[file_name]['status'][defines.ARTELLA_SYNC_WORKING_ASSET_STATUS]['path'] = file_folder_working
                    self._files[file_name]['status'][defines.ARTELLA_SYNC_WORKING_ASSET_STATUS]['file'] = layout_files[0]
            else:
...
class ArtellaSequenceFileWidget(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        main_frame.setStyleSheet("#background {border-radius: 3px;border-style: solid;border-width: 1px;border-color: rgb(32,32,32);}")
        ...
        self._new_version_btn.setIcon(artellapipe.resource.icon('upload'))
